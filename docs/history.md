...
| **v12.1** | **formal 12.1.0 closure** | **precise durable Worker replay; context-index overflow convergence; cross-zlib package identity; five-workload exact-base/exact-head paired performance; machine-verifiable two-attempt closeout and expected-head merge authority** |
...
v12.1 re-opened the parts of that governance claim that were still prose-only. Durable update recovery now replays only unresolved transient tabs and retires terminal browser pages; candidate-index overflow has a one-obligation-per-context-epoch convergence owner and a real-Chrome tail discriminator; package bytes are bound to one checked-in manifest across compressor implementations; performance compares base and candidate on one host across positive, idle, churn, hidden and multi-tab paths; and two same-head receipts plus live PR expected-head comparison now own final merge authorization.
