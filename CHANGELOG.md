...
- Replaced the fragile manual deterministic-test command with **auto-discovery**. Audit proved `tests/classless-decision.mjs` existed but was omitted from the old manual `npm test` chain; v12 now auto-registers 27 deterministic gates and recovers that 6,000-case classless DecisionKernel property proof.
