...
function boot(tabIds=[1,2]){
  ...
  vm.runInNewContext(source,{chrome,console,Promise,Map,Set,Date,Error,Number,String,Array,Object,JSON,Math,setTimeout,clearTimeout});
  const send=(msg,sender={tab:{id:1},frameId:0,documentId:'d',documentLifecycle:'active'})=>new Promise(resolve=>listener(msg,sender,resolve));
  return{listener,installed,calls,send};
}
const origin='https://example.test';
const profile={version:'8.0.0',flows:[{fingerprint:'/login|auth',locator:{hosts:[],selector:'#agree'},descriptor:{kind:'native',severity:0,legal:true,assent:true,required:true,auth:true,linkBucket:1},successes:2,failures:0,ts:Date.now()}]};
let a=boot();
...
session.set('__auto_agree_update_rehydrate__',{version:'8.0.0',ts:Date.now()});
