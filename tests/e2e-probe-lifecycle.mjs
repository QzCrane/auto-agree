...
const FIXTURE = fs.readFileSync(path.join(ROOT, 'tests', 'fixtures', 'regressions', 'probe-lifecycle.html'), 'utf8');
...
await withServer(async url => {
  ...
  try {
    ...
    // Exercise multiple lifecycle generations before the correctness-sensitive hidden insertion.
    for (let i = 0; i < 4; i++) {
    ...
    await page.evaluate(() => {
      ...
      label.id = 'probe-lifecycle-row';
      label.innerHTML = '<input id="probe-lifecycle-agree" type="checkbox" required>I have read and agree to the Terms of Service';
    ...
    const hiddenState = await page.$eval('#probe-lifecycle-agree', el => ({
      checked: el.checked,
      clicks: Number(el.dataset.clicks || 0),
      visibility: document.visibilityState
    }));
    assert.deepEqual(hiddenState, {checked: false, clicks: 0, visibility: 'hidden'}, 'hidden Probe generation must quiesce and never click');

    const hiddenWorlds = await extensionWorldSentinels(page);
    assert.ok(hiddenWorlds.some(world => world.probe === VERSION && !world.gate && !world.engine), 'hidden page must remain Probe-only before resume');

    await page.bringToFront();
    await poll(async () => await page.evaluate(() => document.visibilityState === 'visible'));
    await page.waitForFunction(() => document.querySelector('#probe-lifecycle-agree')?.checked === true, {timeout: 6000});

    const resumed = await page.$eval('#probe-lifecycle-agree', el => ({checked: el.checked, clicks: Number(el.dataset.clicks || 0)}));
    assert.deepEqual(resumed, {checked: true, clicks: 1}, 'visible resumed lifecycle must discover and activate exactly once');

    console.log('e2e-probe-lifecycle:', JSON.stringify({hiddenState, resumed, cycles: 5}));
    console.log('e2e-probe-lifecycle: PASS');
  } finally {
    await browser.close();
  }
});
