(() => {
  ...
  globalThis.__AUTO_AGREE_ENGINE__ = '3.0.0';
  ...
  const CONSEQUENTIAL_LOCAL = /(?:授权扣款|授權扣款|直接扣款|购买协议|購買協議|订单协议|訂單協議|贷款协议|貸款協議|信贷协议|信貸協議|投资风险|投資風險|交易授权|交易授權|投保确认|投保確認|知情同意|劳动合同|勞動合同|雇佣合同|仲裁|放弃.{0,12}权利|放棄.{0,12}權利|集体诉讼|集體訴訟|生物识别.{0,10}(?:同意|授权|授權)|(?:同意|授权|授權).{0,10}生物识别|生物識別.{0,10}(?:同意|授權)|(?:同意|授權).{0,10}生物識別|人脸识别.{0,10}(?:同意|授权|授權)|(?:同意|授权|授權).{0,10}人脸识别|人臉識別.{0,10}(?:同意|授權)|(?:同意|授權).{0,10}人臉識別|授权书|授權書|担保|擔保|purchase\s+agreement|order\s+agreement|terms?\s+of\s+sale|authori[sz]e.{0,20}(?:payment|debit|charge)|direct\s+debit|(?:loan|credit)\s+agreement|investment\s+risk|trading\s+authori[sz]ation|insurance\s+(?:application|purchase|policy\s+application)|medical\s+consent|informed\s+consent|consent\s+to\s+(?:surgery|treatment)|employment\s+(?:agreement|contract)|electronic\s+signature|e-?sign(?:ature)?|auto.?renew|subscription\s+plan|arbitration|waiv(?:e|er)|class\s+action|biometric.{0,16}(?:consent|authori[sz])|(?:consent|authori[sz]).{0,16}biometric|facial\s+recognition.{0,16}(?:consent|authori[sz])|(?:consent|authori[sz]).{0,16}facial\s+recognition|power\s+of\s+attorney|guarant(?:or|ee))/iu;
    ...
    if (root) {
      boundedElementWalk(root, 260, node => {
        ...
        if (node.matches?.('button,input[type="submit"],[role="button"]')) {
          ...
          if (PROCEED.test(label)) {
            if (node.matches?.(':disabled') || node.hasAttribute('disabled') || node.getAttribute('aria-disabled') === 'true') proceedDisabled = true;
          }
        }
      });
    }

    // A disabled button is only weak gating evidence when credentials are not themselves incomplete.
    const gatingScore = proceedDisabled && !credentialIncomplete ? 2 : (proceedDisabled ? 0 : 0);
    return { root, text, auth, transaction, proceedDisabled, credentialIncomplete, gatingScore };
  }
  ...
  function safeSelector(el) {
    if (!(el instanceof Element) || !(el.getRootNode?.() instanceof Document)) return null;
  ...
  function recordSuccess(s) {
    if (!chrome.storage?.local || !location.origin || location.origin === 'null') return;
    const target = s.input || s.control;
    const selector = safeSelector(target);
    if (!selector) return;
    const now = Date.now();
    // The selector is only an acceleration hint. Avoid rewriting identical cache records on
    // every login; refresh an unchanged hint at most once per week.
    if (siteCacheRecord?.selector === selector && siteCacheRecord?.version === VERSION &&
        now - Number(siteCacheRecord.ts || 0) < 7 * 24 * 60 * 60 * 1000) return;
    const record = { selector, ts: now, version: VERSION };
    siteCacheRecord = record;
    chrome.storage.local.set({ [`site:${location.origin}`]: record }).catch?.(() => {});
  }

  function verifyAfterClick(s, before, attempt = 0) {
    setTimeout(() => {
      if (!s.control.isConnected) return;
      const fresh = snapshotCandidate(s.control);
      if (fresh.state.known && fresh.state.checked) {
        clickMemo.set(s.control, { time: performance.now(), succeeded: true });
        recordSuccess(fresh);
        return;
      }
      // Retry once only for state channels that are explicit and still verifiably false.
      if (attempt === 0 && before.known && !before.checked && ['native','aria','data'].includes(before.kind) && fresh.state.known && !fresh.state.checked) {
        ...
        if (target && performance.now() - (clickMemo.get(s.control)?.time || 0) >= 150) {
          try { target.click(); } catch (_) { return; }
          clickMemo.set(s.control, { time: performance.now(), succeeded: false, retry: true });
          verifyAfterClick(fresh, fresh.state, 1);
        }
      }
    }, attempt === 0 ? 180 : 220);
  }

  function performClick(s, overrideTarget = null) {
    ...
    // Native and explicit ARIA/state controls do not need a style/layout read before click.
    // Avoiding getComputedStyle here is crucial immediately after large DOM mutations. CSS-only
    // hidden templates may be toggled early, but routine legal consent is still re-validated and
    // no submit/proceed action is triggered. Geometry/classless fallbacks remain visibility-gated.
    if (!(s.input || s.confidence >= 4)) {
    ...
    verifyAfterClick(s, s.state, 0);
    return true;
  }

  function processCandidate(el, urgent = false) {
    if (!(el instanceof Element) || !isCheckboxLike(el)) return;
  ...
  function processAgreementAnchor(anchor, urgent = false) {
    ...
    const control = knownControlIn(activeRow);
    if (control) return processCandidate(control, urgent);
    activeRow = expandAgreementContainer(activeRow);
    const expandedControl = knownControlIn(activeRow);
    if (expandedControl) return processCandidate(expandedControl, urgent);
    ...
    if (!(pseudo.state.known && pseudo.state.checked)) performClick(pseudo, visual);
  }

  function ownHint(el) {
    if (!(el instanceof Element)) return '';
    return normalize(`${el.getAttribute('aria-label') || ''} ${el.getAttribute('title') || ''} ${el.getAttribute('placeholder') || ''}`, 520);
  }
  ...
  function processElement(el, urgent = false) {
    ...
    if (AUTH.test(hint)) { broadShadowEnabled = true; queueShadowSweep(el); }
  }

  function processTextNode(node, urgent = false) {
    const data = node?.data;
    if (!data || data.length > 1400) return;
    if (AUTH.test(data)) { broadShadowEnabled = true; if (node.parentElement) queueShadowSweep(node.parentElement); }
  ...
  function processSubtree(root, urgent = false, syncBudget = null) {
    ...
    if (budget <= 0) {
      ...
      if (more) { walkJobs.push(job); scheduleBackground(); }
    }
  ...
  function queueShadowSweep(root) {
    ...
    const walker = document.createTreeWalker(root, NodeFilter.SHOW_ELEMENT);
    shadowJobs.push({ root, walker, includeRoot: root instanceof HTMLElement });
    scheduleBackground();
  }
    ...
    try {
      ...
      while ((rootBatches.length || walkJobs.length || batchJobs.length || shadowJobs.length) && rounds++ < 24) {
        if (rootBatches.length) {
          ...
          if (!job.root.isConnected && !(job.root instanceof Document || job.root instanceof ShadowRoot)) walkJobs.shift();
          else if (!runWalkJob(job, BACKGROUND_BUDGET_MS)) walkJobs.shift();
          ...
          if (!job.root.isConnected && !(job.root instanceof Document || job.root instanceof ShadowRoot)) shadowJobs.shift();
          else if (!runShadowJob(job, BACKGROUND_BUDGET_MS)) shadowJobs.shift();
        }
  ...
  function onMutations(records) {
    ...
    for (const record of records) {
      if (record.type === 'childList') {
        ...
        if (nodes.length > 96) {
          // Observer callback stays shallow: head/tail roots are queued, not semantically parsed.
  ...
  function observeRoot(root) {
    ...
    try {
      mutationObserver.observe(root, {
        ...
        characterData: true,
        attributes: true,
        attributeFilter: ['checked','required','disabled','hidden','role','title','aria-checked','aria-required','aria-disabled','aria-hidden','aria-label','aria-labelledby','aria-describedby','data-state','data-checked']
      });
    } catch (_) { return; }
  ...
  function tryCacheFastPath() {
    if (!chrome.storage?.local || !location.origin || location.origin === 'null') return;
    chrome.storage.local.get(`site:${location.origin}`).then(data => {
      const record = data?.[`site:${location.origin}`];
      if (!record?.selector || Date.now() - Number(record.ts || 0) > CACHE_TTL_MS) return;
      siteCacheRecord = record;
      let el = null;
      try { el = document.querySelector(record.selector); } catch (_) {}
  ...
  function boot() {
    ...
    addEventListener('focusin', event => {
      ...
      if (CREDENTIAL.test(hint)) {
        ...
        queueShadowSweep(contextRoot(target) || document.documentElement);
        processIndexedContext(contextRoot(target));
      }
    }, true);
