...
const matches=kernel.split(oldLiteral).length-1;
assert.equal(matches,1,'RuntimeKernel must contain exactly one v11 birth-generation literal');
kernel=kernel.replace(oldLiteral,`const VERSION = '${NEXT}';`);
fs.writeFileSync(kernelFile,kernel);

console.log(`v12-release-cut: PASS ${OLD} -> ${NEXT}`);
